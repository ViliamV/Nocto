...
def test_test_command_input_2() -> None:
    ...
    # Passing dotenv (without all variables)
    result = RUNNER.invoke(app, ["test", str(HERE / "data" / "input_2.txt"), "--dotenv"])
    assert result.exit_code == 1
    assert "Missing environment variables" in result.stderr
    ...
    result = RUNNER.invoke(
        app, ["test", str(HERE / "data" / "input_2.txt"), "--dotenv", "--dotenv-file", str(HERE / "dotenv_test")]
    )
    assert result.exit_code == 0
    assert result.stdout == ""
...
def test_missing_filter() -> None:
    result = RUNNER.invoke(
        app, ["test", str(HERE / "data" / "input_3.txt"), "--dotenv", "--dotenv-file", str(HERE / "dotenv_test")]
    )
    assert result.exit_code == 1
...
def test_replace_command(input_name: str, output_name: str) -> None:
    result = RUNNER.invoke(
        app, ["replace", str(HERE / "data" / input_name), "--dotenv", "--dotenv-file", str(HERE / "dotenv_test")]
    )
    assert result.exit_code == 0
